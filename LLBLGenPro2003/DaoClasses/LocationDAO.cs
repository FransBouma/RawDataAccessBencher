...
{
	/// <summary>
	/// General DAO class for the Location Entity. It will perform database oriented actions for
	/// a entity of type 'LocationEntity'. This DAO works on an EntityFields object. 
	/// </summary>
	public class LocationDAO : DaoBase
	{
		/// <summary>
		/// CTor
		/// </summary>
		...
		/// <summary>
		/// Adds the given fields to the database as a new 'Location' entity.
		/// </summary>
		...
		public bool AddLocation(IEntityFields fields, ITransaction containingTransaction)
		{
			if(fields==null)
			{
		...
		/// <summary>
		/// Updates an entity of type 'LocationEntity' using the given fields. 
		/// </summary>
		...
		/// <exception cref="ArgumentNullException">When fields is null</exception>
		public bool UpdateLocation(IEntityFields fields, ITransaction containingTransaction)
		{
			if(fields==null)
			{
				throw new ArgumentNullException("fields", "fields can't be null");
			...
			updateFilter.Add(new FieldCompareValuePredicate(fields[(int)LocationFieldIndex.LocationID], ComparisonOperator.Equal));
		...
		/// <returns>true if the update was succesful, false otherwise</returns>
		/// <exception cref="ArgumentNullException">When fields is null</exception>
		public bool UpdateLocation(IEntityFields fields, ITransaction containingTransaction, IPredicate updateRestriction)
		{
			if(fields==null)
			{
				throw new ArgumentNullException("fields", "fields can't be null");
		...
		/// <summary>
		/// Reads the data of an entity of type 'LocationEntity' data into EntityFields object and returns that object. Which data is read is determined using
		...
		public IEntityFields FetchLocation(IEntityFields fieldsToFetch, ITransaction containingTransaction, System.Int16 locationID)
		{
			fieldsToFetch.State = EntityState.New;
			IPredicateExpression selectFilter = new PredicateExpression();
			selectFilter.Add(new FieldCompareValuePredicate(fieldsToFetch[(int)LocationFieldIndex.LocationID], ComparisonOperator.Equal, locationID));
		...
		/// <summary>
		/// Deletes an entity of type 'LocationEntity' from the persistent storage. Which entity is deleted is determined from the passed in EntityFields object.
		/// </summary>
		...
		/// <exception cref="ArgumentNullException">When fields is null</exception>
		public bool DeleteLocation(IEntityFields fields, ITransaction containingTransaction, IPredicate deleteRestriction)
		{
			if(fields==null)
			{
				throw new ArgumentNullException("fields", "fields can't be null");
			...
			deleteFilter.Add(new FieldCompareValuePredicate(fields[(int)LocationFieldIndex.LocationID], ComparisonOperator.Equal));
		...
		{
			IEntityFields fieldsToReturn = EntityFieldsFactory.CreateEntityFieldsObject(EntityType.LocationEntity);
		...
		/// <summary>
		/// Retrieves entities of the type 'LocationEntity' in a datatable which match the specified filter. 
			...
			ISortExpression sortClauses, IEntityFactory entityFactoryToUse, IValidator validatorToUse, ProductEntity productInstance)
		{
			...
			relations.Add(LocationEntity.Relations.ProductInventoryEntityUsingLocationID);
			relations.Add(ProductInventoryEntity.Relations.ProductEntityUsingProductID);
		...
		/// <returns>true if succeeded, false otherwise</returns>
		public bool GetMultiUsingWorkOrder(ITransaction containingTransaction, IEntityCollection collectionToFill, long maxNumberOfItemsToReturn, 
			ISortExpression sortClauses, IEntityFactory entityFactoryToUse, IValidator validatorToUse, WorkOrderEntity workOrderInstance)
		{
			IEntityFields fieldsToReturn = EntityFieldsFactory.CreateEntityFieldsObject(EntityType.LocationEntity);

			RelationCollection relations = new RelationCollection();
			// add the relations to walk.
			relations.Add(LocationEntity.Relations.WorkOrderRoutingEntityUsingLocationID);
			relations.Add(WorkOrderRoutingEntity.Relations.WorkOrderEntityUsingWorkOrderID);
			
			IPredicateExpression selectFilter = new PredicateExpression();
			selectFilter.Add(new FieldCompareValuePredicate(workOrderInstance.Fields[(int)WorkOrderFieldIndex.WorkOrderID], ComparisonOperator.Equal, workOrderInstance.WorkOrderID));
			CreateAndRunMultiInstanceRetrievalQuery(fieldsToReturn, containingTransaction, collectionToFill, maxNumberOfItemsToReturn, sortClauses, 
				entityFactoryToUse, selectFilter, validatorToUse, relations);
			return true;
		}
		...
		/// <summary>
		/// Deletes from the persistent storage all 'Location' entities which match with the specified filter, formulated in
		...
		{
			IEntityFields fields = EntityFieldsFactory.CreateEntityFieldsObject(EntityType.LocationEntity);
