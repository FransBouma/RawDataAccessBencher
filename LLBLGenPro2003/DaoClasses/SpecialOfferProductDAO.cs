...
{
	/// <summary>
	/// General DAO class for the SpecialOfferProduct Entity. It will perform database oriented actions for
	/// a entity of type 'SpecialOfferProductEntity'. This DAO works on an EntityFields object. 
	/// </summary>
	public class SpecialOfferProductDAO : DaoBase
	{
		/// <summary>
		/// CTor
		/// </summary>
		public SpecialOfferProductDAO()
		{
		}

		/// <summary>
		/// Adds the given fields to the database as a new 'SpecialOfferProduct' entity.
		/// </summary>
		...
		public bool AddSpecialOfferProduct(IEntityFields fields, ITransaction containingTransaction)
		{
			if(fields==null)
			{
		...
		/// <summary>
		/// Updates an entity of type 'SpecialOfferProductEntity' using the given fields. 
		/// </summary>
		...
		/// <exception cref="ArgumentNullException">When fields is null</exception>
		public bool UpdateSpecialOfferProduct(IEntityFields fields, ITransaction containingTransaction)
		{
			if(fields==null)
			{
				throw new ArgumentNullException("fields", "fields can't be null");
			...
			updateFilter.Add(new FieldCompareValuePredicate(fields[(int)SpecialOfferProductFieldIndex.SpecialOfferID], ComparisonOperator.Equal));
			updateFilter.Add(new FieldCompareValuePredicate(fields[(int)SpecialOfferProductFieldIndex.ProductID], ComparisonOperator.Equal));
		...
		/// <returns>true if the update was succesful, false otherwise</returns>
		/// <exception cref="ArgumentNullException">When fields is null</exception>
		public bool UpdateSpecialOfferProduct(IEntityFields fields, ITransaction containingTransaction, IPredicate updateRestriction)
		{
			if(fields==null)
			{
				throw new ArgumentNullException("fields", "fields can't be null");
		...
		/// <summary>
		/// Reads the data of an entity of type 'SpecialOfferProductEntity' data into EntityFields object and returns that object. Which data is read is determined using
		...
		public IEntityFields FetchSpecialOfferProduct(IEntityFields fieldsToFetch, ITransaction containingTransaction, System.Int32 specialOfferID, System.Int32 productID)
		{
			fieldsToFetch.State = EntityState.New;
			IPredicateExpression selectFilter = new PredicateExpression();
			selectFilter.Add(new FieldCompareValuePredicate(fieldsToFetch[(int)SpecialOfferProductFieldIndex.SpecialOfferID], ComparisonOperator.Equal, specialOfferID));
			selectFilter.Add(new FieldCompareValuePredicate(fieldsToFetch[(int)SpecialOfferProductFieldIndex.ProductID], ComparisonOperator.Equal, productID));
		...
		/// <summary>
		/// Deletes an entity of type 'SpecialOfferProductEntity' from the persistent storage. Which entity is deleted is determined from the passed in EntityFields object.
		/// </summary>
		...
		/// <exception cref="ArgumentNullException">When fields is null</exception>
		public bool DeleteSpecialOfferProduct(IEntityFields fields, ITransaction containingTransaction, IPredicate deleteRestriction)
		{
			if(fields==null)
			{
				throw new ArgumentNullException("fields", "fields can't be null");
			...
			deleteFilter.Add(new FieldCompareValuePredicate(fields[(int)SpecialOfferProductFieldIndex.SpecialOfferID], ComparisonOperator.Equal));
			deleteFilter.Add(new FieldCompareValuePredicate(fields[(int)SpecialOfferProductFieldIndex.ProductID], ComparisonOperator.Equal));
		...
		/// <summary>
		/// Retrieves in the calling SpecialOfferProductCollection object all SpecialOfferProductEntity objects which have data in common
		...
		public bool GetMulti(ITransaction containingTransaction, IEntityCollection collectionToFill, long maxNumberOfItemsToReturn, ISortExpression sortClauses, 
				IEntityFactory entityFactoryToUse, IValidator validatorToUse, IPredicateExpression filter, ProductEntity productInstance, SpecialOfferEntity specialOfferInstance)
		{
			IEntityFields fieldsToReturn = EntityFieldsFactory.CreateEntityFieldsObject(EntityType.SpecialOfferProductEntity);
			IPredicateExpression selectFilter = CreateFilterUsingForeignKeys(productInstance, specialOfferInstance, fieldsToReturn);
			if(filter!=null)
			{
		...
		/// <summary>
		/// Retrieves in the calling SpecialOfferProductCollection object all SpecialOfferProductEntity objects
		...
		/// <summary>
		/// Retrieves entities of the type 'SpecialOfferProductEntity' in a datatable which match the specified filter. 
		...
		/// <returns>true if succeeded, false otherwise</returns>
		public bool GetMultiUsingSalesOrderHeader(ITransaction containingTransaction, IEntityCollection collectionToFill, long maxNumberOfItemsToReturn, 
			ISortExpression sortClauses, IEntityFactory entityFactoryToUse, IValidator validatorToUse, SalesOrderHeaderEntity salesOrderHeaderInstance)
		{
			IEntityFields fieldsToReturn = EntityFieldsFactory.CreateEntityFieldsObject(EntityType.SpecialOfferProductEntity);

			RelationCollection relations = new RelationCollection();
			// add the relations to walk.
			relations.Add(SpecialOfferProductEntity.Relations.SalesOrderDetailEntityUsingSpecialOfferIDProductID);
			relations.Add(SalesOrderDetailEntity.Relations.SalesOrderHeaderEntityUsingSalesOrderID);
			
			IPredicateExpression selectFilter = new PredicateExpression();
			selectFilter.Add(new FieldCompareValuePredicate(salesOrderHeaderInstance.Fields[(int)SalesOrderHeaderFieldIndex.SalesOrderID], ComparisonOperator.Equal, salesOrderHeaderInstance.SalesOrderID));
			CreateAndRunMultiInstanceRetrievalQuery(fieldsToReturn, containingTransaction, collectionToFill, maxNumberOfItemsToReturn, sortClauses, 
				entityFactoryToUse, selectFilter, validatorToUse, relations);
			return true;
		}
		...
		/// <summary>
		/// Deletes from the persistent storage all 'SpecialOfferProduct' entities which have data in common
		/// with the specified related Entities. If one is omitted, that entity is not used as a filter. 
		/// </summary>
		/// <param name="containingTransaction">A containing transaction, if caller is added to a transaction, or null if not.</param>
		/// <param name="productInstance">ProductEntity instance to use as a filter for the SpecialOfferProductEntity objects to delete</param>
		/// <param name="specialOfferInstance">SpecialOfferEntity instance to use as a filter for the SpecialOfferProductEntity objects to delete</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int DeleteMulti(ITransaction containingTransaction, ProductEntity productInstance, SpecialOfferEntity specialOfferInstance)
		{
			IEntityFields fields = EntityFieldsFactory.CreateEntityFieldsObject(EntityType.SpecialOfferProductEntity);
			IPredicateExpression deleteFilter = CreateFilterUsingForeignKeys(productInstance, specialOfferInstance, fields);
			return CreateAndRunDeleteQuery(fields[0], containingTransaction, deleteFilter);
		}
	

		/// <summary>
		/// Deletes from the persistent storage all 'SpecialOfferProduct' entities which match with the specified filter, formulated in
		...
		/// <param name="containingTransaction">A containing transaction, if caller is added to a transaction, or null if not.</param>
		/// <param name="productInstance">ProductEntity instance to use as a filter for the SpecialOfferProductEntity objects to update</param>
		/// <param name="specialOfferInstance">SpecialOfferEntity instance to use as a filter for the SpecialOfferProductEntity objects to update</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMulti(IEntityFields fieldsWithNewValues, ITransaction containingTransaction, ProductEntity productInstance, SpecialOfferEntity specialOfferInstance)
		{
			...
			IEntityFields fields = EntityFieldsFactory.CreateEntityFieldsObject(EntityType.SpecialOfferProductEntity);
			IPredicateExpression updateFilter = CreateFilterUsingForeignKeys(productInstance, specialOfferInstance, fields);
		...
		/// </summary>
		/// <param name="productInstance">ProductEntity instance to use as a filter for the SpecialOfferProductEntity objects</param>
		/// <param name="specialOfferInstance">SpecialOfferEntity instance to use as a filter for the SpecialOfferProductEntity objects</param>
		/// <param name="fieldsToReturn">IEntityFields implementation which forms the definition of the fieldset of the target entity.</param>
		/// <returns>A ready to use PredicateExpression based on the passed in foreign key value holders.</returns>
		private IPredicateExpression CreateFilterUsingForeignKeys(ProductEntity productInstance, SpecialOfferEntity specialOfferInstance, IEntityFields fieldsToReturn)
		{
			IPredicateExpression selectFilter = new PredicateExpression();
			
			if(productInstance != null)
			{
				selectFilter.Add(new FieldCompareValuePredicate(fieldsToReturn[(int)SpecialOfferProductFieldIndex.ProductID], ComparisonOperator.Equal, productInstance.ProductID));
			}
			
			if(specialOfferInstance != null)
			{
				selectFilter.Add(new FieldCompareValuePredicate(fieldsToReturn[(int)SpecialOfferProductFieldIndex.SpecialOfferID], ComparisonOperator.Equal, specialOfferInstance.SpecialOfferID));
			}
