...
{
	/// <summary>
	/// General DAO class for the Individual Entity. It will perform database oriented actions for
	/// a entity of type 'IndividualEntity'. This DAO works on an EntityFields object. 
	/// </summary>
	public class IndividualDAO : DaoBase
	{
		/// <summary>
		/// CTor
		/// </summary>
		...
		/// <summary>
		/// Adds the given fields to the database as a new 'Individual' entity.
		/// </summary>
		...
		public bool AddIndividual(IEntityFields fields, ITransaction containingTransaction)
		{
			if(fields==null)
			{
		...
		/// <summary>
		/// Updates an entity of type 'IndividualEntity' using the given fields. 
		/// </summary>
		...
		/// <exception cref="ArgumentNullException">When fields is null</exception>
		public bool UpdateIndividual(IEntityFields fields, ITransaction containingTransaction)
		{
			if(fields==null)
			{
				throw new ArgumentNullException("fields", "fields can't be null");
			...
			updateFilter.Add(new FieldCompareValuePredicate(fields[(int)IndividualFieldIndex.CustomerID], ComparisonOperator.Equal));
		...
		/// <returns>true if the update was succesful, false otherwise</returns>
		/// <exception cref="ArgumentNullException">When fields is null</exception>
		public bool UpdateIndividual(IEntityFields fields, ITransaction containingTransaction, IPredicate updateRestriction)
		{
			if(fields==null)
			{
				throw new ArgumentNullException("fields", "fields can't be null");
		...
		/// <summary>
		/// Reads the data of an entity of type 'IndividualEntity' data into EntityFields object and returns that object. Which data is read is determined using
		...
		public IEntityFields FetchIndividual(IEntityFields fieldsToFetch, ITransaction containingTransaction, System.Int32 customerID)
		{
			fieldsToFetch.State = EntityState.New;
			IPredicateExpression selectFilter = new PredicateExpression();
			selectFilter.Add(new FieldCompareValuePredicate(fieldsToFetch[(int)IndividualFieldIndex.CustomerID], ComparisonOperator.Equal, customerID));
		...
		/// <summary>
		/// Deletes an entity of type 'IndividualEntity' from the persistent storage. Which entity is deleted is determined from the passed in EntityFields object.
		/// </summary>
		...
		/// <exception cref="ArgumentNullException">When fields is null</exception>
		public bool DeleteIndividual(IEntityFields fields, ITransaction containingTransaction, IPredicate deleteRestriction)
		{
			if(fields==null)
			{
				throw new ArgumentNullException("fields", "fields can't be null");
			...
			deleteFilter.Add(new FieldCompareValuePredicate(fields[(int)IndividualFieldIndex.CustomerID], ComparisonOperator.Equal));
		...
		public bool GetMulti(ITransaction containingTransaction, IEntityCollection collectionToFill, long maxNumberOfItemsToReturn, ISortExpression sortClauses, 
				IEntityFactory entityFactoryToUse, IValidator validatorToUse, IPredicateExpression filter, ContactEntity contactInstance)
		{
			IEntityFields fieldsToReturn = EntityFieldsFactory.CreateEntityFieldsObject(EntityType.IndividualEntity);
		...
		/// <summary>
		/// Retrieves in the calling IndividualCollection object all IndividualEntity objects
		...
		/// <summary>
		/// Retrieves entities of the type 'IndividualEntity' in a datatable which match the specified filter. 
		...
		/// <summary>
		/// Deletes from the persistent storage all 'Individual' entities which have data in common
		/// with the specified related Entities. If one is omitted, that entity is not used as a filter. 
		/// </summary>
		/// <param name="containingTransaction">A containing transaction, if caller is added to a transaction, or null if not.</param>
		...
		public int DeleteMulti(ITransaction containingTransaction, ContactEntity contactInstance)
		{
			IEntityFields fields = EntityFieldsFactory.CreateEntityFieldsObject(EntityType.IndividualEntity);
			IPredicateExpression deleteFilter = CreateFilterUsingForeignKeys(contactInstance, fields);
			return CreateAndRunDeleteQuery(fields[0], containingTransaction, deleteFilter);
		}
	

		/// <summary>
		/// Deletes from the persistent storage all 'Individual' entities which match with the specified filter, formulated in
		...
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMulti(IEntityFields fieldsWithNewValues, ITransaction containingTransaction, ContactEntity contactInstance)
		{
			...
			IEntityFields fields = EntityFieldsFactory.CreateEntityFieldsObject(EntityType.IndividualEntity);
			IPredicateExpression updateFilter = CreateFilterUsingForeignKeys(contactInstance, fields);
		...
		private IPredicateExpression CreateFilterUsingForeignKeys(ContactEntity contactInstance, IEntityFields fieldsToReturn)
		{
			IPredicateExpression selectFilter = new PredicateExpression();
			
			if(contactInstance != null)
			{
				selectFilter.Add(new FieldCompareValuePredicate(fieldsToReturn[(int)IndividualFieldIndex.ContactID], ComparisonOperator.Equal, contactInstance.ContactID));
			}
