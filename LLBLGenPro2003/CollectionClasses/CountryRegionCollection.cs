...
{
	/// <summary>
	/// Collection class for storing and retrieving collections of CountryRegionEntity objects. 
	/// </summary>
	[Serializable]
	public class CountryRegionCollection : EntityCollectionBase
	{
		/// <summary>
		/// CTor
		/// </summary>
		public CountryRegionCollection():base(new PropertyDescriptorFactory(), typeof(CountryRegionCollection), new CountryRegionEntityFactory())
		{
		}
		...
		public CountryRegionCollection(IPropertyDescriptorFactory propertyDescriptorFactoryToUse, IEntityFactory entityFactoryToUse)
			:base(propertyDescriptorFactoryToUse, typeof(CountryRegionCollection), entityFactoryToUse)
		{
		}
		...
		public CountryRegionCollection(IPropertyDescriptorFactory propertyDescriptorFactoryToUse, IEntityFactory entityFactoryToUse, CountryRegionValidator validatorToUse)
			:base(propertyDescriptorFactoryToUse, typeof(CountryRegionCollection), entityFactoryToUse, validatorToUse)
		{
		}
		...
		private CountryRegionCollection(SerializationInfo info, StreamingContext context) : base(info, context)
		{
		}
		...
		{
			return base.ToXml("CountryRegionCollection");
		}

	
	
		/// <summary>
		/// Retrieves in this CountryRegionCollection object all CountryRegionEntity objects which are related via a 
		...
		/// <summary>
		/// Retrieves in this CountryRegionCollection object all CountryRegionEntity objects which are related via a 
		/// Relation of type 'm:n' with the passed in CurrencyEntity. 
		/// All current elements in the collection are removed from the collection.
		/// </summary>
		/// <param name="currencyInstance">CurrencyEntity object to be used as a filter in the m:n relation</param>
		/// <returns>true if the retrieval succeeded, false otherwise</returns>
		public bool GetMultiManyToManyUsingCurrency(CurrencyEntity currencyInstance)
		{
			return GetMultiManyToManyUsingCurrency(currencyInstance, base.MaxNumberOfItemsToReturn, base.SortClauses);
		}
		...
		/// <param name="sortClauses">The order by specifications for the sorting of the resultset. When not specified, no sorting is applied.</param>
		/// <returns>true if the retrieval succeeded, false otherwise</returns>
		public bool GetMultiManyToManyUsingCurrency(CurrencyEntity currencyInstance, long maxNumberOfItemsToReturn, ISortExpression sortClauses)
		{
			if(!base.SuppressClearInGetMulti)
			{
				this.Clear();
			...
			CountryRegionDAO dao = DAOFactory.CreateCountryRegionDAO();
			return dao.GetMultiUsingCurrency(base.Transaction, this, maxNumberOfItemsToReturn, sortClauses, base.EntityFactoryToUse, base.ValidatorToUse, currencyInstance);
		}
	

		/// <summary>
		/// Retrieves in this CountryRegionCollection object all CountryRegionEntity objects which match with the specified filter, formulated in
		...
		/// <summary>
		/// Retrieves CountryRegionEntity rows in a datatable which match the specified filter. It will always create a new connection to the database.
		/// </summary>
		/// <param name="selectFilter">A predicate or predicate expression which should be used as filter for the entities to retrieve.</param>
		/// <param name="maxNumberOfItemsToReturn"> The maximum number of items to return with this retrieval query.</param>
		/// <param name="sortClauses">The order by specifications for the sorting of the resultset. When not specified, no sorting is applied.</param>
		...
		/// will result in a query removing all CountryRegion entities from the persistent storage</param>
		...
		/// <summary>
		/// Updates in the persistent storage all entities which have data in common with the specified CountryRegionEntity. 
		...
		/// </summary>
		/// <param name="entityWithNewValues">CountryRegionEntity instance which holds the new values for the matching entities to update. Only
		...
		/// will result in an update action which will affect all CountryRegion entities.</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMulti(CountryRegionEntity entityWithNewValues, IPredicate updateFilter)
		{
			CountryRegionDAO dao = DAOFactory.CreateCountryRegionDAO();
		...
		/// <param name="relations">The set of relations to walk to construct the total query.</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMulti(CountryRegionEntity entityWithNewValues, IPredicate updateFilter, IRelationCollection relations)
		{
			CountryRegionDAO dao = DAOFactory.CreateCountryRegionDAO();
		...
		/// </summary>
		public CountryRegionEntity this [int index]
		{
			get { return (CountryRegionEntity)List[index]; }
		}
		#endregion
