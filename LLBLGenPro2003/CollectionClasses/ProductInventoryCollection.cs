...
{
	/// <summary>
	/// Collection class for storing and retrieving collections of ProductInventoryEntity objects. 
	/// </summary>
	[Serializable]
	public class ProductInventoryCollection : EntityCollectionBase
	{
		/// <summary>
		/// CTor
		/// </summary>
		public ProductInventoryCollection():base(new PropertyDescriptorFactory(), typeof(ProductInventoryCollection), new ProductInventoryEntityFactory())
		{
		}
		...
		public ProductInventoryCollection(IPropertyDescriptorFactory propertyDescriptorFactoryToUse, IEntityFactory entityFactoryToUse)
			:base(propertyDescriptorFactoryToUse, typeof(ProductInventoryCollection), entityFactoryToUse)
		{
		}
		...
		public ProductInventoryCollection(IPropertyDescriptorFactory propertyDescriptorFactoryToUse, IEntityFactory entityFactoryToUse, ProductInventoryValidator validatorToUse)
			:base(propertyDescriptorFactoryToUse, typeof(ProductInventoryCollection), entityFactoryToUse, validatorToUse)
		{
		}
		...
		private ProductInventoryCollection(SerializationInfo info, StreamingContext context) : base(info, context)
		{
		}
		...
		{
			return base.ToXml("ProductInventoryCollection");
		}

	
		/// <summary>
		/// Retrieves in this ProductInventoryCollection object all ProductInventoryEntity objects which have data in common
		...
		public bool GetMultiManyToOne(LocationEntity locationInstance, ProductEntity productInstance)
		{
			return GetMultiManyToOne(locationInstance, productInstance, base.MaxNumberOfItemsToReturn, base.SortClauses, null);
		}
		...
		/// <returns>true if succeeded, false otherwise</returns>
		public bool GetMultiManyToOne(LocationEntity locationInstance, ProductEntity productInstance, IPredicateExpression filter)
		{
			return GetMultiManyToOne(locationInstance, productInstance, base.MaxNumberOfItemsToReturn, base.SortClauses, filter);
		}
		...
		/// <param name="filter">Extra filter to limit the resultset. Predicate expression can be null, in which case it will be ignored.</param>
		/// <returns>true if succeeded, false otherwise</returns>
		public bool GetMultiManyToOne(LocationEntity locationInstance, ProductEntity productInstance, long maxNumberOfItemsToReturn, ISortExpression sortClauses, IPredicateExpression filter)
		{
			...
			return dao.GetMulti(base.Transaction, this, maxNumberOfItemsToReturn, sortClauses, base.EntityFactoryToUse, base.ValidatorToUse, filter, locationInstance, productInstance);
		}
		...
		public int DeleteMultiManyToOne( LocationEntity locationInstance,  ProductEntity productInstance)
		{
			ProductInventoryDAO dao = DAOFactory.CreateProductInventoryDAO();
		...
		/// <summary>
		/// Updates in the persistent storage all ProductInventory entities which have data in common with the specified related Entities.
		/// If one is omitted, that entity is not used as a filter.
		...
		/// </summary>
		/// <param name="entityWithNewValues">ProductInventoryEntity instance which holds the new values for the matching entities to update. Only
		/// changed fields are taken into account</param>
		/// <param name="locationInstance">LocationEntity instance to use as a filter for the ProductInventoryEntity objects to return</param>
		/// <param name="productInstance">ProductEntity instance to use as a filter for the ProductInventoryEntity objects to return</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMultiManyToOne(ProductInventoryEntity entityWithNewValues, LocationEntity locationInstance, ProductEntity productInstance)
		{
			ProductInventoryDAO dao = DAOFactory.CreateProductInventoryDAO();
			return dao.UpdateMulti(entityWithNewValues.Fields, base.Transaction, locationInstance, productInstance);
		}
	
	

		/// <summary>
		/// Retrieves in this ProductInventoryCollection object all ProductInventoryEntity objects which match with the specified filter, formulated in
		...
		/// <summary>
		/// Retrieves ProductInventoryEntity rows in a datatable which match the specified filter. It will always create a new connection to the database.
		/// </summary>
		/// <param name="selectFilter">A predicate or predicate expression which should be used as filter for the entities to retrieve.</param>
		/// <param name="maxNumberOfItemsToReturn"> The maximum number of items to return with this retrieval query.</param>
		/// <param name="sortClauses">The order by specifications for the sorting of the resultset. When not specified, no sorting is applied.</param>
		...
		/// will result in a query removing all ProductInventory entities from the persistent storage</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		...
		/// <summary>
		/// Updates in the persistent storage all entities which have data in common with the specified ProductInventoryEntity. 
		...
		/// will result in an update action which will affect all ProductInventory entities.</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMulti(ProductInventoryEntity entityWithNewValues, IPredicate updateFilter)
		{
			ProductInventoryDAO dao = DAOFactory.CreateProductInventoryDAO();
		...
		/// <param name="relations">The set of relations to walk to construct the total query.</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMulti(ProductInventoryEntity entityWithNewValues, IPredicate updateFilter, IRelationCollection relations)
		{
			ProductInventoryDAO dao = DAOFactory.CreateProductInventoryDAO();
		...
		/// </summary>
		public ProductInventoryEntity this [int index]
		{
			get { return (ProductInventoryEntity)List[index]; }
		}
		#endregion
