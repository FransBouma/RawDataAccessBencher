...
{
	/// <summary>
	/// Collection class for storing and retrieving collections of ErrorLogEntity objects. 
	/// </summary>
	[Serializable]
	public class ErrorLogCollection : EntityCollectionBase
	{
		/// <summary>
		/// CTor
		/// </summary>
		public ErrorLogCollection():base(new PropertyDescriptorFactory(), typeof(ErrorLogCollection), new ErrorLogEntityFactory())
		{
		}
		...
		public ErrorLogCollection(IPropertyDescriptorFactory propertyDescriptorFactoryToUse, IEntityFactory entityFactoryToUse)
			:base(propertyDescriptorFactoryToUse, typeof(ErrorLogCollection), entityFactoryToUse)
		{
		}
		...
		public ErrorLogCollection(IPropertyDescriptorFactory propertyDescriptorFactoryToUse, IEntityFactory entityFactoryToUse, ErrorLogValidator validatorToUse)
			:base(propertyDescriptorFactoryToUse, typeof(ErrorLogCollection), entityFactoryToUse, validatorToUse)
		{
		}
		...
		private ErrorLogCollection(SerializationInfo info, StreamingContext context) : base(info, context)
		{
		}
		...
		{
			return base.ToXml("ErrorLogCollection");
		}
		...
		/// <summary>
		/// Retrieves in this ErrorLogCollection object all ErrorLogEntity objects which match with the specified filter, formulated in
		...
		/// <summary>
		/// Retrieves ErrorLogEntity rows in a datatable which match the specified filter. It will always create a new connection to the database.
		/// </summary>
		/// <param name="selectFilter">A predicate or predicate expression which should be used as filter for the entities to retrieve.</param>
		/// <param name="maxNumberOfItemsToReturn"> The maximum number of items to return with this retrieval query.</param>
		/// <param name="sortClauses">The order by specifications for the sorting of the resultset. When not specified, no sorting is applied.</param>
		...
		/// will result in a query removing all ErrorLog entities from the persistent storage</param>
		...
		/// <summary>
		/// Updates in the persistent storage all entities which have data in common with the specified ErrorLogEntity. 
		...
		/// </summary>
		/// <param name="entityWithNewValues">ErrorLogEntity instance which holds the new values for the matching entities to update. Only
		...
		/// will result in an update action which will affect all ErrorLog entities.</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMulti(ErrorLogEntity entityWithNewValues, IPredicate updateFilter)
		{
			ErrorLogDAO dao = DAOFactory.CreateErrorLogDAO();
		...
		/// <param name="relations">The set of relations to walk to construct the total query.</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMulti(ErrorLogEntity entityWithNewValues, IPredicate updateFilter, IRelationCollection relations)
		{
			ErrorLogDAO dao = DAOFactory.CreateErrorLogDAO();
		...
		/// </summary>
		public ErrorLogEntity this [int index]
		{
			get { return (ErrorLogEntity)List[index]; }
		}
		#endregion
