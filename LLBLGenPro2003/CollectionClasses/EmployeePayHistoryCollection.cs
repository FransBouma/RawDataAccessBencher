...
{
	/// <summary>
	/// Collection class for storing and retrieving collections of EmployeePayHistoryEntity objects. 
	/// </summary>
	[Serializable]
	public class EmployeePayHistoryCollection : EntityCollectionBase
	{
		/// <summary>
		/// CTor
		/// </summary>
		public EmployeePayHistoryCollection():base(new PropertyDescriptorFactory(), typeof(EmployeePayHistoryCollection), new EmployeePayHistoryEntityFactory())
		{
		}
		...
		public EmployeePayHistoryCollection(IPropertyDescriptorFactory propertyDescriptorFactoryToUse, IEntityFactory entityFactoryToUse)
			:base(propertyDescriptorFactoryToUse, typeof(EmployeePayHistoryCollection), entityFactoryToUse)
		{
		}
		...
		public EmployeePayHistoryCollection(IPropertyDescriptorFactory propertyDescriptorFactoryToUse, IEntityFactory entityFactoryToUse, EmployeePayHistoryValidator validatorToUse)
			:base(propertyDescriptorFactoryToUse, typeof(EmployeePayHistoryCollection), entityFactoryToUse, validatorToUse)
		{
		}
		...
		private EmployeePayHistoryCollection(SerializationInfo info, StreamingContext context) : base(info, context)
		{
		}
		...
		{
			return base.ToXml("EmployeePayHistoryCollection");
		}

	
		/// <summary>
		/// Retrieves in this EmployeePayHistoryCollection object all EmployeePayHistoryEntity objects which have data in common
		...
		public bool GetMultiManyToOne(EmployeeEntity employeeInstance)
		{
			return GetMultiManyToOne(employeeInstance, base.MaxNumberOfItemsToReturn, base.SortClauses, null);
		}
		...
		/// <returns>true if succeeded, false otherwise</returns>
		public bool GetMultiManyToOne(EmployeeEntity employeeInstance, IPredicateExpression filter)
		{
			return GetMultiManyToOne(employeeInstance, base.MaxNumberOfItemsToReturn, base.SortClauses, filter);
		}
		...
		/// <param name="filter">Extra filter to limit the resultset. Predicate expression can be null, in which case it will be ignored.</param>
		/// <returns>true if succeeded, false otherwise</returns>
		public bool GetMultiManyToOne(EmployeeEntity employeeInstance, long maxNumberOfItemsToReturn, ISortExpression sortClauses, IPredicateExpression filter)
		{
			...
			return dao.GetMulti(base.Transaction, this, maxNumberOfItemsToReturn, sortClauses, base.EntityFactoryToUse, base.ValidatorToUse, filter, employeeInstance);
		}
		...
		public int DeleteMultiManyToOne( EmployeeEntity employeeInstance)
		{
			EmployeePayHistoryDAO dao = DAOFactory.CreateEmployeePayHistoryDAO();
			return dao.DeleteMulti(base.Transaction, employeeInstance);
		}


		/// <summary>
		/// Updates in the persistent storage all EmployeePayHistory entities which have data in common with the specified related Entities.
		/// If one is omitted, that entity is not used as a filter.
		...
		/// </summary>
		/// <param name="entityWithNewValues">EmployeePayHistoryEntity instance which holds the new values for the matching entities to update. Only
		/// changed fields are taken into account</param>
		/// <param name="employeeInstance">EmployeeEntity instance to use as a filter for the EmployeePayHistoryEntity objects to return</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMultiManyToOne(EmployeePayHistoryEntity entityWithNewValues, EmployeeEntity employeeInstance)
		{
			EmployeePayHistoryDAO dao = DAOFactory.CreateEmployeePayHistoryDAO();
			return dao.UpdateMulti(entityWithNewValues.Fields, base.Transaction, employeeInstance);
		}
	
	

		/// <summary>
		/// Retrieves in this EmployeePayHistoryCollection object all EmployeePayHistoryEntity objects which match with the specified filter, formulated in
		...
		/// <summary>
		/// Retrieves EmployeePayHistoryEntity rows in a datatable which match the specified filter. It will always create a new connection to the database.
		/// </summary>
		/// <param name="selectFilter">A predicate or predicate expression which should be used as filter for the entities to retrieve.</param>
		/// <param name="maxNumberOfItemsToReturn"> The maximum number of items to return with this retrieval query.</param>
		/// <param name="sortClauses">The order by specifications for the sorting of the resultset. When not specified, no sorting is applied.</param>
		...
		/// will result in a query removing all EmployeePayHistory entities from the persistent storage</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		...
		/// <summary>
		/// Updates in the persistent storage all entities which have data in common with the specified EmployeePayHistoryEntity. 
		...
		/// will result in an update action which will affect all EmployeePayHistory entities.</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMulti(EmployeePayHistoryEntity entityWithNewValues, IPredicate updateFilter)
		{
			EmployeePayHistoryDAO dao = DAOFactory.CreateEmployeePayHistoryDAO();
		...
		/// <param name="relations">The set of relations to walk to construct the total query.</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMulti(EmployeePayHistoryEntity entityWithNewValues, IPredicate updateFilter, IRelationCollection relations)
		{
			EmployeePayHistoryDAO dao = DAOFactory.CreateEmployeePayHistoryDAO();
		...
		/// </summary>
		public EmployeePayHistoryEntity this [int index]
		{
			get { return (EmployeePayHistoryEntity)List[index]; }
		}
		#endregion
