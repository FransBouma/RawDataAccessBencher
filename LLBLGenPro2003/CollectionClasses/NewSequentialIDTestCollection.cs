...
{
	/// <summary>
	/// Collection class for storing and retrieving collections of NewSequentialIDTestEntity objects. 
	/// </summary>
	[Serializable]
	public class NewSequentialIDTestCollection : EntityCollectionBase
	{
		/// <summary>
		/// CTor
		/// </summary>
		public NewSequentialIDTestCollection():base(new PropertyDescriptorFactory(), typeof(NewSequentialIDTestCollection), new NewSequentialIDTestEntityFactory())
		{
		}
		...
		public NewSequentialIDTestCollection(IPropertyDescriptorFactory propertyDescriptorFactoryToUse, IEntityFactory entityFactoryToUse)
			:base(propertyDescriptorFactoryToUse, typeof(NewSequentialIDTestCollection), entityFactoryToUse)
		{
		}
		...
		public NewSequentialIDTestCollection(IPropertyDescriptorFactory propertyDescriptorFactoryToUse, IEntityFactory entityFactoryToUse, NewSequentialIDTestValidator validatorToUse)
			:base(propertyDescriptorFactoryToUse, typeof(NewSequentialIDTestCollection), entityFactoryToUse, validatorToUse)
		{
		}
		...
		private NewSequentialIDTestCollection(SerializationInfo info, StreamingContext context) : base(info, context)
		{
		}
		...
		{
			return base.ToXml("NewSequentialIDTestCollection");
		}
		...
		/// <summary>
		/// Retrieves in this NewSequentialIDTestCollection object all NewSequentialIDTestEntity objects which match with the specified filter, formulated in
		...
		/// <summary>
		/// Retrieves NewSequentialIDTestEntity rows in a datatable which match the specified filter. It will always create a new connection to the database.
		/// </summary>
		/// <param name="selectFilter">A predicate or predicate expression which should be used as filter for the entities to retrieve.</param>
		/// <param name="maxNumberOfItemsToReturn"> The maximum number of items to return with this retrieval query.</param>
		/// <param name="sortClauses">The order by specifications for the sorting of the resultset. When not specified, no sorting is applied.</param>
		...
		/// will result in a query removing all NewSequentialIDTest entities from the persistent storage</param>
		...
		/// <summary>
		/// Updates in the persistent storage all entities which have data in common with the specified NewSequentialIDTestEntity. 
		...
		/// </summary>
		/// <param name="entityWithNewValues">NewSequentialIDTestEntity instance which holds the new values for the matching entities to update. Only
		...
		/// will result in an update action which will affect all NewSequentialIDTest entities.</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMulti(NewSequentialIDTestEntity entityWithNewValues, IPredicate updateFilter)
		{
			NewSequentialIDTestDAO dao = DAOFactory.CreateNewSequentialIDTestDAO();
		...
		/// <param name="relations">The set of relations to walk to construct the total query.</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMulti(NewSequentialIDTestEntity entityWithNewValues, IPredicate updateFilter, IRelationCollection relations)
		{
			NewSequentialIDTestDAO dao = DAOFactory.CreateNewSequentialIDTestDAO();
		...
		{
			return "NewSequentialIDTestCollection";
		}
		...
		/// </summary>
		public NewSequentialIDTestEntity this [int index]
		{
			get { return (NewSequentialIDTestEntity)List[index]; }
		}
		#endregion
