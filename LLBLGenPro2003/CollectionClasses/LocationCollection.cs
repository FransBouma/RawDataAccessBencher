...
{
	/// <summary>
	/// Collection class for storing and retrieving collections of LocationEntity objects. 
	/// </summary>
	[Serializable]
	public class LocationCollection : EntityCollectionBase
	{
		/// <summary>
		/// CTor
		/// </summary>
		public LocationCollection():base(new PropertyDescriptorFactory(), typeof(LocationCollection), new LocationEntityFactory())
		{
		}
		...
		public LocationCollection(IPropertyDescriptorFactory propertyDescriptorFactoryToUse, IEntityFactory entityFactoryToUse)
			:base(propertyDescriptorFactoryToUse, typeof(LocationCollection), entityFactoryToUse)
		{
		}
		...
		public LocationCollection(IPropertyDescriptorFactory propertyDescriptorFactoryToUse, IEntityFactory entityFactoryToUse, LocationValidator validatorToUse)
			:base(propertyDescriptorFactoryToUse, typeof(LocationCollection), entityFactoryToUse, validatorToUse)
		{
		}
		...
		private LocationCollection(SerializationInfo info, StreamingContext context) : base(info, context)
		{
		}
		...
		{
			return base.ToXml("LocationCollection");
		}

	
	
		/// <summary>
		/// Retrieves in this LocationCollection object all LocationEntity objects which are related via a 
		...
		/// <summary>
		/// Retrieves in this LocationCollection object all LocationEntity objects which are related via a 
		/// Relation of type 'm:n' with the passed in WorkOrderEntity. 
		/// All current elements in the collection are removed from the collection.
		/// </summary>
		/// <param name="workOrderInstance">WorkOrderEntity object to be used as a filter in the m:n relation</param>
		/// <returns>true if the retrieval succeeded, false otherwise</returns>
		public bool GetMultiManyToManyUsingWorkOrder(WorkOrderEntity workOrderInstance)
		{
			return GetMultiManyToManyUsingWorkOrder(workOrderInstance, base.MaxNumberOfItemsToReturn, base.SortClauses);
		}
		...
		/// <param name="sortClauses">The order by specifications for the sorting of the resultset. When not specified, no sorting is applied.</param>
		/// <returns>true if the retrieval succeeded, false otherwise</returns>
		public bool GetMultiManyToManyUsingWorkOrder(WorkOrderEntity workOrderInstance, long maxNumberOfItemsToReturn, ISortExpression sortClauses)
		{
			if(!base.SuppressClearInGetMulti)
			{
				this.Clear();
			...
			LocationDAO dao = DAOFactory.CreateLocationDAO();
			return dao.GetMultiUsingWorkOrder(base.Transaction, this, maxNumberOfItemsToReturn, sortClauses, base.EntityFactoryToUse, base.ValidatorToUse, workOrderInstance);
		}
	

		/// <summary>
		/// Retrieves in this LocationCollection object all LocationEntity objects which match with the specified filter, formulated in
		...
		/// <summary>
		/// Retrieves LocationEntity rows in a datatable which match the specified filter. It will always create a new connection to the database.
		/// </summary>
		/// <param name="selectFilter">A predicate or predicate expression which should be used as filter for the entities to retrieve.</param>
		/// <param name="maxNumberOfItemsToReturn"> The maximum number of items to return with this retrieval query.</param>
		/// <param name="sortClauses">The order by specifications for the sorting of the resultset. When not specified, no sorting is applied.</param>
		...
		/// will result in a query removing all Location entities from the persistent storage</param>
		...
		/// <summary>
		/// Updates in the persistent storage all entities which have data in common with the specified LocationEntity. 
		...
		/// </summary>
		/// <param name="entityWithNewValues">LocationEntity instance which holds the new values for the matching entities to update. Only
		...
		/// will result in an update action which will affect all Location entities.</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMulti(LocationEntity entityWithNewValues, IPredicate updateFilter)
		{
			LocationDAO dao = DAOFactory.CreateLocationDAO();
		...
		/// <param name="relations">The set of relations to walk to construct the total query.</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMulti(LocationEntity entityWithNewValues, IPredicate updateFilter, IRelationCollection relations)
		{
			LocationDAO dao = DAOFactory.CreateLocationDAO();
		...
		/// </summary>
		public LocationEntity this [int index]
		{
			get { return (LocationEntity)List[index]; }
		}
		#endregion
