...
{
	/// <summary>
	/// Collection class for storing and retrieving collections of ShiftEntity objects. 
	/// </summary>
	[Serializable]
	public class ShiftCollection : EntityCollectionBase
	{
		/// <summary>
		/// CTor
		/// </summary>
		public ShiftCollection():base(new PropertyDescriptorFactory(), typeof(ShiftCollection), new ShiftEntityFactory())
		{
		}
		...
		public ShiftCollection(IPropertyDescriptorFactory propertyDescriptorFactoryToUse, IEntityFactory entityFactoryToUse)
			:base(propertyDescriptorFactoryToUse, typeof(ShiftCollection), entityFactoryToUse)
		{
		}
		...
		public ShiftCollection(IPropertyDescriptorFactory propertyDescriptorFactoryToUse, IEntityFactory entityFactoryToUse, ShiftValidator validatorToUse)
			:base(propertyDescriptorFactoryToUse, typeof(ShiftCollection), entityFactoryToUse, validatorToUse)
		{
		}
		...
		private ShiftCollection(SerializationInfo info, StreamingContext context) : base(info, context)
		{
		}
		...
		{
			return base.ToXml("ShiftCollection");
		}

	
	
		/// <summary>
		/// Retrieves in this ShiftCollection object all ShiftEntity objects which are related via a 
		...
		/// <summary>
		/// Retrieves in this ShiftCollection object all ShiftEntity objects which match with the specified filter, formulated in
		...
		/// <summary>
		/// Retrieves ShiftEntity rows in a datatable which match the specified filter. It will always create a new connection to the database.
		/// </summary>
		/// <param name="selectFilter">A predicate or predicate expression which should be used as filter for the entities to retrieve.</param>
		/// <param name="maxNumberOfItemsToReturn"> The maximum number of items to return with this retrieval query.</param>
		/// <param name="sortClauses">The order by specifications for the sorting of the resultset. When not specified, no sorting is applied.</param>
		...
		/// will result in a query removing all Shift entities from the persistent storage</param>
		...
		/// <summary>
		/// Updates in the persistent storage all entities which have data in common with the specified ShiftEntity. 
		...
		/// </summary>
		/// <param name="entityWithNewValues">ShiftEntity instance which holds the new values for the matching entities to update. Only
		...
		/// will result in an update action which will affect all Shift entities.</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMulti(ShiftEntity entityWithNewValues, IPredicate updateFilter)
		{
			ShiftDAO dao = DAOFactory.CreateShiftDAO();
		...
		/// <param name="relations">The set of relations to walk to construct the total query.</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMulti(ShiftEntity entityWithNewValues, IPredicate updateFilter, IRelationCollection relations)
		{
			ShiftDAO dao = DAOFactory.CreateShiftDAO();
		...
		/// </summary>
		public ShiftEntity this [int index]
		{
			get { return (ShiftEntity)List[index]; }
		}
		#endregion
