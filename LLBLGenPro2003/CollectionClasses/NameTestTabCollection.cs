...
{
	/// <summary>
	/// Collection class for storing and retrieving collections of NameTestTabEntity objects. 
	/// </summary>
	[Serializable]
	public class NameTestTabCollection : EntityCollectionBase
	{
		/// <summary>
		/// CTor
		/// </summary>
		public NameTestTabCollection():base(new PropertyDescriptorFactory(), typeof(NameTestTabCollection), new NameTestTabEntityFactory())
		{
		}
		...
		public NameTestTabCollection(IPropertyDescriptorFactory propertyDescriptorFactoryToUse, IEntityFactory entityFactoryToUse)
			:base(propertyDescriptorFactoryToUse, typeof(NameTestTabCollection), entityFactoryToUse)
		{
		}
		...
		public NameTestTabCollection(IPropertyDescriptorFactory propertyDescriptorFactoryToUse, IEntityFactory entityFactoryToUse, NameTestTabValidator validatorToUse)
			:base(propertyDescriptorFactoryToUse, typeof(NameTestTabCollection), entityFactoryToUse, validatorToUse)
		{
		}
		...
		private NameTestTabCollection(SerializationInfo info, StreamingContext context) : base(info, context)
		{
		}
		...
		{
			return base.ToXml("NameTestTabCollection");
		}
		...
		/// <summary>
		/// Retrieves in this NameTestTabCollection object all NameTestTabEntity objects which match with the specified filter, formulated in
		...
		/// <summary>
		/// Retrieves NameTestTabEntity rows in a datatable which match the specified filter. It will always create a new connection to the database.
		/// </summary>
		/// <param name="selectFilter">A predicate or predicate expression which should be used as filter for the entities to retrieve.</param>
		/// <param name="maxNumberOfItemsToReturn"> The maximum number of items to return with this retrieval query.</param>
		/// <param name="sortClauses">The order by specifications for the sorting of the resultset. When not specified, no sorting is applied.</param>
		...
		/// will result in a query removing all NameTestTab entities from the persistent storage</param>
		...
		/// <summary>
		/// Updates in the persistent storage all entities which have data in common with the specified NameTestTabEntity. 
		...
		/// </summary>
		/// <param name="entityWithNewValues">NameTestTabEntity instance which holds the new values for the matching entities to update. Only
		...
		/// will result in an update action which will affect all NameTestTab entities.</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMulti(NameTestTabEntity entityWithNewValues, IPredicate updateFilter)
		{
			NameTestTabDAO dao = DAOFactory.CreateNameTestTabDAO();
		...
		/// <param name="relations">The set of relations to walk to construct the total query.</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMulti(NameTestTabEntity entityWithNewValues, IPredicate updateFilter, IRelationCollection relations)
		{
			NameTestTabDAO dao = DAOFactory.CreateNameTestTabDAO();
		...
		/// </summary>
		public NameTestTabEntity this [int index]
		{
			get { return (NameTestTabEntity)List[index]; }
		}
		#endregion
