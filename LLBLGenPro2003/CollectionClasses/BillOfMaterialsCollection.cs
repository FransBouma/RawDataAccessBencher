...
{
	/// <summary>
	/// Collection class for storing and retrieving collections of BillOfMaterialsEntity objects. 
	/// </summary>
	[Serializable]
	public class BillOfMaterialsCollection : EntityCollectionBase
	{
		/// <summary>
		/// CTor
		/// </summary>
		public BillOfMaterialsCollection():base(new PropertyDescriptorFactory(), typeof(BillOfMaterialsCollection), new BillOfMaterialsEntityFactory())
		{
		}
		...
		public BillOfMaterialsCollection(IPropertyDescriptorFactory propertyDescriptorFactoryToUse, IEntityFactory entityFactoryToUse)
			:base(propertyDescriptorFactoryToUse, typeof(BillOfMaterialsCollection), entityFactoryToUse)
		{
		}
		...
		public BillOfMaterialsCollection(IPropertyDescriptorFactory propertyDescriptorFactoryToUse, IEntityFactory entityFactoryToUse, BillOfMaterialsValidator validatorToUse)
			:base(propertyDescriptorFactoryToUse, typeof(BillOfMaterialsCollection), entityFactoryToUse, validatorToUse)
		{
		}
		...
		private BillOfMaterialsCollection(SerializationInfo info, StreamingContext context) : base(info, context)
		{
		}
		...
		{
			return base.ToXml("BillOfMaterialsCollection");
		}

	
		/// <summary>
		/// Retrieves in this BillOfMaterialsCollection object all BillOfMaterialsEntity objects which have data in common
		...
		public bool GetMultiManyToOne(ProductEntity productInstance, ProductEntity product_Instance, UnitMeasureEntity unitMeasureInstance)
		{
			return GetMultiManyToOne(productInstance, product_Instance, unitMeasureInstance, base.MaxNumberOfItemsToReturn, base.SortClauses, null);
		}
		...
		/// <returns>true if succeeded, false otherwise</returns>
		public bool GetMultiManyToOne(ProductEntity productInstance, ProductEntity product_Instance, UnitMeasureEntity unitMeasureInstance, IPredicateExpression filter)
		{
			return GetMultiManyToOne(productInstance, product_Instance, unitMeasureInstance, base.MaxNumberOfItemsToReturn, base.SortClauses, filter);
		}
		...
		/// <param name="filter">Extra filter to limit the resultset. Predicate expression can be null, in which case it will be ignored.</param>
		/// <returns>true if succeeded, false otherwise</returns>
		public bool GetMultiManyToOne(ProductEntity productInstance, ProductEntity product_Instance, UnitMeasureEntity unitMeasureInstance, long maxNumberOfItemsToReturn, ISortExpression sortClauses, IPredicateExpression filter)
		{
			...
			return dao.GetMulti(base.Transaction, this, maxNumberOfItemsToReturn, sortClauses, base.EntityFactoryToUse, base.ValidatorToUse, filter, productInstance, product_Instance, unitMeasureInstance);
		}
		...
		public int DeleteMultiManyToOne( ProductEntity productInstance,  ProductEntity product_Instance,  UnitMeasureEntity unitMeasureInstance)
		{
			BillOfMaterialsDAO dao = DAOFactory.CreateBillOfMaterialsDAO();
			return dao.DeleteMulti(base.Transaction, productInstance, product_Instance, unitMeasureInstance);
		}


		/// <summary>
		/// Updates in the persistent storage all BillOfMaterials entities which have data in common with the specified related Entities.
		/// If one is omitted, that entity is not used as a filter.
		...
		/// </summary>
		/// <param name="entityWithNewValues">BillOfMaterialsEntity instance which holds the new values for the matching entities to update. Only
		/// changed fields are taken into account</param>
		/// <param name="productInstance">ProductEntity instance to use as a filter for the BillOfMaterialsEntity objects to return</param>
		/// <param name="product_Instance">ProductEntity instance to use as a filter for the BillOfMaterialsEntity objects to return</param>
		/// <param name="unitMeasureInstance">UnitMeasureEntity instance to use as a filter for the BillOfMaterialsEntity objects to return</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMultiManyToOne(BillOfMaterialsEntity entityWithNewValues, ProductEntity productInstance, ProductEntity product_Instance, UnitMeasureEntity unitMeasureInstance)
		{
			BillOfMaterialsDAO dao = DAOFactory.CreateBillOfMaterialsDAO();
			return dao.UpdateMulti(entityWithNewValues.Fields, base.Transaction, productInstance, product_Instance, unitMeasureInstance);
		}
	
	

		/// <summary>
		/// Retrieves in this BillOfMaterialsCollection object all BillOfMaterialsEntity objects which match with the specified filter, formulated in
		...
		/// <summary>
		/// Retrieves BillOfMaterialsEntity rows in a datatable which match the specified filter. It will always create a new connection to the database.
		/// </summary>
		/// <param name="selectFilter">A predicate or predicate expression which should be used as filter for the entities to retrieve.</param>
		/// <param name="maxNumberOfItemsToReturn"> The maximum number of items to return with this retrieval query.</param>
		/// <param name="sortClauses">The order by specifications for the sorting of the resultset. When not specified, no sorting is applied.</param>
		...
		/// will result in a query removing all BillOfMaterials entities from the persistent storage</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		...
		/// <summary>
		/// Updates in the persistent storage all entities which have data in common with the specified BillOfMaterialsEntity. 
		...
		/// will result in an update action which will affect all BillOfMaterials entities.</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMulti(BillOfMaterialsEntity entityWithNewValues, IPredicate updateFilter)
		{
			BillOfMaterialsDAO dao = DAOFactory.CreateBillOfMaterialsDAO();
		...
		/// <param name="relations">The set of relations to walk to construct the total query.</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMulti(BillOfMaterialsEntity entityWithNewValues, IPredicate updateFilter, IRelationCollection relations)
		{
			BillOfMaterialsDAO dao = DAOFactory.CreateBillOfMaterialsDAO();
		...
		/// </summary>
		public BillOfMaterialsEntity this [int index]
		{
			get { return (BillOfMaterialsEntity)List[index]; }
		}
		#endregion
