...
{
	/// <summary>
	/// Collection class for storing and retrieving collections of ProductDocumentEntity objects. 
	/// </summary>
	[Serializable]
	public class ProductDocumentCollection : EntityCollectionBase
	{
		/// <summary>
		/// CTor
		/// </summary>
		public ProductDocumentCollection():base(new PropertyDescriptorFactory(), typeof(ProductDocumentCollection), new ProductDocumentEntityFactory())
		{
		}
		...
		public ProductDocumentCollection(IPropertyDescriptorFactory propertyDescriptorFactoryToUse, IEntityFactory entityFactoryToUse)
			:base(propertyDescriptorFactoryToUse, typeof(ProductDocumentCollection), entityFactoryToUse)
		{
		}
		...
		public ProductDocumentCollection(IPropertyDescriptorFactory propertyDescriptorFactoryToUse, IEntityFactory entityFactoryToUse, ProductDocumentValidator validatorToUse)
			:base(propertyDescriptorFactoryToUse, typeof(ProductDocumentCollection), entityFactoryToUse, validatorToUse)
		{
		}
		...
		private ProductDocumentCollection(SerializationInfo info, StreamingContext context) : base(info, context)
		{
		}
		...
		{
			return base.ToXml("ProductDocumentCollection");
		}

	
		/// <summary>
		/// Retrieves in this ProductDocumentCollection object all ProductDocumentEntity objects which have data in common
		...
		public bool GetMultiManyToOne(DocumentEntity documentInstance, ProductEntity productInstance)
		{
			return GetMultiManyToOne(documentInstance, productInstance, base.MaxNumberOfItemsToReturn, base.SortClauses, null);
		}
		...
		/// <returns>true if succeeded, false otherwise</returns>
		public bool GetMultiManyToOne(DocumentEntity documentInstance, ProductEntity productInstance, IPredicateExpression filter)
		{
			return GetMultiManyToOne(documentInstance, productInstance, base.MaxNumberOfItemsToReturn, base.SortClauses, filter);
		}
		...
		/// <param name="filter">Extra filter to limit the resultset. Predicate expression can be null, in which case it will be ignored.</param>
		/// <returns>true if succeeded, false otherwise</returns>
		public bool GetMultiManyToOne(DocumentEntity documentInstance, ProductEntity productInstance, long maxNumberOfItemsToReturn, ISortExpression sortClauses, IPredicateExpression filter)
		{
			...
			return dao.GetMulti(base.Transaction, this, maxNumberOfItemsToReturn, sortClauses, base.EntityFactoryToUse, base.ValidatorToUse, filter, documentInstance, productInstance);
		}
		...
		public int DeleteMultiManyToOne( DocumentEntity documentInstance,  ProductEntity productInstance)
		{
			ProductDocumentDAO dao = DAOFactory.CreateProductDocumentDAO();
			return dao.DeleteMulti(base.Transaction, documentInstance, productInstance);
		}


		/// <summary>
		/// Updates in the persistent storage all ProductDocument entities which have data in common with the specified related Entities.
		/// If one is omitted, that entity is not used as a filter.
		...
		/// </summary>
		/// <param name="entityWithNewValues">ProductDocumentEntity instance which holds the new values for the matching entities to update. Only
		/// changed fields are taken into account</param>
		/// <param name="documentInstance">DocumentEntity instance to use as a filter for the ProductDocumentEntity objects to return</param>
		/// <param name="productInstance">ProductEntity instance to use as a filter for the ProductDocumentEntity objects to return</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMultiManyToOne(ProductDocumentEntity entityWithNewValues, DocumentEntity documentInstance, ProductEntity productInstance)
		{
			ProductDocumentDAO dao = DAOFactory.CreateProductDocumentDAO();
			return dao.UpdateMulti(entityWithNewValues.Fields, base.Transaction, documentInstance, productInstance);
		}
	
	

		/// <summary>
		/// Retrieves in this ProductDocumentCollection object all ProductDocumentEntity objects which match with the specified filter, formulated in
		...
		/// <summary>
		/// Retrieves ProductDocumentEntity rows in a datatable which match the specified filter. It will always create a new connection to the database.
		/// </summary>
		/// <param name="selectFilter">A predicate or predicate expression which should be used as filter for the entities to retrieve.</param>
		/// <param name="maxNumberOfItemsToReturn"> The maximum number of items to return with this retrieval query.</param>
		/// <param name="sortClauses">The order by specifications for the sorting of the resultset. When not specified, no sorting is applied.</param>
		...
		/// will result in a query removing all ProductDocument entities from the persistent storage</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		...
		/// <summary>
		/// Updates in the persistent storage all entities which have data in common with the specified ProductDocumentEntity. 
		...
		/// will result in an update action which will affect all ProductDocument entities.</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMulti(ProductDocumentEntity entityWithNewValues, IPredicate updateFilter)
		{
			ProductDocumentDAO dao = DAOFactory.CreateProductDocumentDAO();
		...
		/// <param name="relations">The set of relations to walk to construct the total query.</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMulti(ProductDocumentEntity entityWithNewValues, IPredicate updateFilter, IRelationCollection relations)
		{
			ProductDocumentDAO dao = DAOFactory.CreateProductDocumentDAO();
		...
		/// </summary>
		public ProductDocumentEntity this [int index]
		{
			get { return (ProductDocumentEntity)List[index]; }
		}
		#endregion
