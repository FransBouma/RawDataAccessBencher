...
{
	/// <summary>
	/// Collection class for storing and retrieving collections of SequentialIDSubTypeEntity objects. 
	/// </summary>
	[Serializable]
	public class SequentialIDSubTypeCollection : EntityCollectionBase
	{
		/// <summary>
		/// CTor
		/// </summary>
		public SequentialIDSubTypeCollection():base(new PropertyDescriptorFactory(), typeof(SequentialIDSubTypeCollection), new SequentialIDSubTypeEntityFactory())
		{
		}
		...
		public SequentialIDSubTypeCollection(IPropertyDescriptorFactory propertyDescriptorFactoryToUse, IEntityFactory entityFactoryToUse)
			:base(propertyDescriptorFactoryToUse, typeof(SequentialIDSubTypeCollection), entityFactoryToUse)
		{
		}
		...
		public SequentialIDSubTypeCollection(IPropertyDescriptorFactory propertyDescriptorFactoryToUse, IEntityFactory entityFactoryToUse, SequentialIDSubTypeValidator validatorToUse)
			:base(propertyDescriptorFactoryToUse, typeof(SequentialIDSubTypeCollection), entityFactoryToUse, validatorToUse)
		{
		}
		...
		private SequentialIDSubTypeCollection(SerializationInfo info, StreamingContext context) : base(info, context)
		{
		}
		...
		{
			return base.ToXml("SequentialIDSubTypeCollection");
		}
		...
		/// <summary>
		/// Retrieves in this SequentialIDSubTypeCollection object all SequentialIDSubTypeEntity objects which match with the specified filter, formulated in
		...
		/// <summary>
		/// Retrieves SequentialIDSubTypeEntity rows in a datatable which match the specified filter. It will always create a new connection to the database.
		/// </summary>
		/// <param name="selectFilter">A predicate or predicate expression which should be used as filter for the entities to retrieve.</param>
		/// <param name="maxNumberOfItemsToReturn"> The maximum number of items to return with this retrieval query.</param>
		/// <param name="sortClauses">The order by specifications for the sorting of the resultset. When not specified, no sorting is applied.</param>
		...
		/// will result in a query removing all SequentialIDSubType entities from the persistent storage</param>
		...
		/// <summary>
		/// Updates in the persistent storage all entities which have data in common with the specified SequentialIDSubTypeEntity. 
		...
		/// </summary>
		/// <param name="entityWithNewValues">SequentialIDSubTypeEntity instance which holds the new values for the matching entities to update. Only
		...
		/// will result in an update action which will affect all SequentialIDSubType entities.</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMulti(SequentialIDSubTypeEntity entityWithNewValues, IPredicate updateFilter)
		{
			SequentialIDSubTypeDAO dao = DAOFactory.CreateSequentialIDSubTypeDAO();
		...
		/// <param name="relations">The set of relations to walk to construct the total query.</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMulti(SequentialIDSubTypeEntity entityWithNewValues, IPredicate updateFilter, IRelationCollection relations)
		{
			SequentialIDSubTypeDAO dao = DAOFactory.CreateSequentialIDSubTypeDAO();
		...
		{
			return "SequentialIDSubTypeCollection";
		}
		...
		/// </summary>
		public SequentialIDSubTypeEntity this [int index]
		{
			get { return (SequentialIDSubTypeEntity)List[index]; }
		}
		#endregion
