...
{
	/// <summary>
	/// Collection class for storing and retrieving collections of WorkOrderEntity objects. 
using SD.LLBLGen.Pro.ORMSupportClasses2003;	[Serializable]
	public class WorkOrderCollection : EntityCollectionBase
	{
		/// <summary>
		/// CTor
		/// </summary>
		public WorkOrderCollection():base(new PropertyDescriptorFactory(), typeof(WorkOrderCollection), new WorkOrderEntityFactory())
		{
		}
		...
		public WorkOrderCollection(IPropertyDescriptorFactory propertyDescriptorFactoryToUse, IEntityFactory entityFactoryToUse)
			:base(propertyDescriptorFactoryToUse, typeof(WorkOrderCollection), entityFactoryToUse)
		{
		}
		...
		public WorkOrderCollection(IPropertyDescriptorFactory propertyDescriptorFactoryToUse, IEntityFactory entityFactoryToUse, WorkOrderValidator validatorToUse)
			:base(propertyDescriptorFactoryToUse, typeof(WorkOrderCollection), entityFactoryToUse, validatorToUse)
		{
		}
		...
		private WorkOrderCollection(SerializationInfo info, StreamingContext context) : base(info, context)
		{
		}
		...
		{
			return base.ToXml("WorkOrderCollection");
		}

	
		/// <summary>
		/// Retrieves in this WorkOrderCollection object all WorkOrderEntity objects which have data in common
		...
		public bool GetMultiManyToOne(ProductEntity productInstance, ScrapReasonEntity scrapReasonInstance)
		{
			return GetMultiManyToOne(productInstance, scrapReasonInstance, base.MaxNumberOfItemsToReturn, base.SortClauses, null);
		}
		...
		/// <returns>true if succeeded, false otherwise</returns>
		public bool GetMultiManyToOne(ProductEntity productInstance, ScrapReasonEntity scrapReasonInstance, IPredicateExpression filter)
		{
			return GetMultiManyToOne(productInstance, scrapReasonInstance, base.MaxNumberOfItemsToReturn, base.SortClauses, filter);
		}
		...
		/// <param name="filter">Extra filter to limit the resultset. Predicate expression can be null, in which case it will be ignored.</param>
		/// <returns>true if succeeded, false otherwise</returns>
		public bool GetMultiManyToOne(ProductEntity productInstance, ScrapReasonEntity scrapReasonInstance, long maxNumberOfItemsToReturn, ISortExpression sortClauses, IPredicateExpression filter)
		{
			...
			return dao.GetMulti(base.Transaction, this, maxNumberOfItemsToReturn, sortClauses, base.EntityFactoryToUse, base.ValidatorToUse, filter, productInstance, scrapReasonInstance);
		}
		...
		public int DeleteMultiManyToOne( ProductEntity productInstance,  ScrapReasonEntity scrapReasonInstance)
		{
			WorkOrderDAO dao = DAOFactory.CreateWorkOrderDAO();
		...
		/// <summary>
		/// Updates in the persistent storage all WorkOrder entities which have data in common with the specified related Entities.
		/// If one is omitted, that entity is not used as a filter.
		...
		/// </summary>
		/// <param name="entityWithNewValues">WorkOrderEntity instance which holds the new values for the matching entities to update. Only
		/// changed fields are taken into account</param>
		/// <param name="productInstance">ProductEntity instance to use as a filter for the WorkOrderEntity objects to return</param>
		/// <param name="scrapReasonInstance">ScrapReasonEntity instance to use as a filter for the WorkOrderEntity objects to return</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMultiManyToOne(WorkOrderEntity entityWithNewValues, ProductEntity productInstance, ScrapReasonEntity scrapReasonInstance)
		{
			WorkOrderDAO dao = DAOFactory.CreateWorkOrderDAO();
			return dao.UpdateMulti(entityWithNewValues.Fields, base.Transaction, productInstance, scrapReasonInstance);
		}
	
	
		/// <summary>
		/// Retrieves in this WorkOrderCollection object all WorkOrderEntity objects which are related via a 
		...
		/// <summary>
		/// Retrieves in this WorkOrderCollection object all WorkOrderEntity objects which match with the specified filter, formulated in
		...
		/// <summary>
		/// Retrieves WorkOrderEntity rows in a datatable which match the specified filter. It will always create a new connection to the database.
		/// </summary>
		/// <param name="selectFilter">A predicate or predicate expression which should be used as filter for the entities to retrieve.</param>
		/// <param name="maxNumberOfItemsToReturn"> The maximum number of items to return with this retrieval query.</param>
		/// <param name="sortClauses">The order by specifications for the sorting of the resultset. When not specified, no sorting is applied.</param>
		...
		/// <summary>
		/// Deletes from the persistent storage all WorkOrder entities which match with the specified filter, formulated in
		/// the predicate or predicate expression definition.
		/// </summary>
		...
		/// will result in a query removing all WorkOrder entities from the persistent storage</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		...
		/// <summary>
		/// Updates in the persistent storage all entities which have data in common with the specified WorkOrderEntity. 
		...
		/// will result in an update action which will affect all WorkOrder entities.</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMulti(WorkOrderEntity entityWithNewValues, IPredicate updateFilter)
		{
			WorkOrderDAO dao = DAOFactory.CreateWorkOrderDAO();
		...
		/// <param name="relations">The set of relations to walk to construct the total query.</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMulti(WorkOrderEntity entityWithNewValues, IPredicate updateFilter, IRelationCollection relations)
		{
			WorkOrderDAO dao = DAOFactory.CreateWorkOrderDAO();
		...
		/// </summary>
		public WorkOrderEntity this [int index]
		{
			get { return (WorkOrderEntity)List[index]; }
		}
		#endregion
