...
namespace PetaPoco
{
  ...
  {
    private const string RXCOLUMNS = @"\A\s*SELECT\s+((?:\((?>\((?<depth>)|\)(?<-depth>)|.?)*(?(depth)(?!))\)|.)*?)(?<!,\s+)\bFROM\b";
    private const string RXORDERBY = @"\bORDER\s+BY\s+(?:\((?>\((?<depth>)|\)(?<-depth>)|.?)*(?(depth)(?!))\)|[\w\(\)\.])+(?:\s+(?:ASC|DESC))?(?:\s*,\s*(?:\((?>\((?<depth>)|\)(?<-depth>)|.?)*(?(depth)(?!))\)|[\w\(\)\.])+(?:\s+(?:ASC|DESC))?)*";
    private const string RXDISTINCT = @"\ADISTINCT\s";
    private const string RXSELECT = @"\A\s*(SELECT|EXECUTE|CALL)\s";
    private const string RXFROM = @"\A\s*FROM\s";
    private const string RXPARAMS = @"(?<!@)@\w+";
    private const string RXPARAMSPREFIX = @"(?<!@)@\w+";
    ...
    {
      return Regex.Replace(_sql, RXPARAMS, m =>
      {
        ...
        if (int.TryParse(param, out paramIndex))
        {
          ...
          if (paramIndex < 0 || paramIndex >= args_src.Length)
            throw new ArgumentOutOfRangeException(
              string.Format("Parameter '@{0}' specified but only {1} parameters supplied (in `{2}`)", paramIndex,
    ...
    void AddParam(IDbCommand cmd, object item, string ParameterPrefix)
    {
      ...
      else
      {
        ...
        else if (t == typeof(AnsiString))
        {
          ...
          p.Size = Math.Max(((AnsiString) item).Value.Length + 1, 4000);
    ...
    public IDbCommand CreateCommand(IDbConnection connection, string sql, params object[] args)
    {
      ...
      if (_paramPrefix != "@")
        sql = Regex.Replace(sql, RXPARAMSPREFIX, m => _paramPrefix + m.Value.Substring(1));
    ...
    string AddSelectClause<T>(string sql)
    {
      ...
      if (!Regex.IsMatch(sql, RXSELECT, RegexOptions.Singleline | RegexOptions.IgnoreCase | RegexOptions.Multiline))
      {
        ...
        if (!Regex.IsMatch(sql, RXFROM, RegexOptions.Singleline | RegexOptions.IgnoreCase | RegexOptions.Multiline))
    ...
    public bool SplitSqlForPaging(string sql, out string sqlCount, out string sqlSelectRemoved, out string sqlOrderBy)
    {
      ...
      var m = Regex.Match(sql, RXCOLUMNS, RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.Singleline);
      if (!m.Success)
        return false;
      ...
      if (Regex.IsMatch(sqlSelectRemoved, RXDISTINCT, RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.Singleline))
      ...
      m = Regex.Match(sqlCount, RXORDERBY, RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.Singleline);
      if (!m.Success)
      {
        sqlOrderBy = null;
      }
    ...
    public void BuildPageQueries<T>(long skip, long take, string sql, ref object[] args, out string sqlCount, out string sqlPage)
    {
      ...
      {
        sqlSelectRemoved = Regex.Replace(sqlSelectRemoved, RXORDERBY, "", RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.Singleline);
        if (Regex.IsMatch(sqlSelectRemoved, RXDISTINCT, RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.Singleline))
        {
          ...
          if (type == typeof(object))
          {
            ...
            for (int i = firstColumn; i < firstColumn + countColumns; i++)
            {
              ...
              if (ForceDateTimesToUtc && converter == null && srcType == typeof(DateTime))
                converter = delegate(object src) { return ((DateTime)src).ToUniversalTime(); };
